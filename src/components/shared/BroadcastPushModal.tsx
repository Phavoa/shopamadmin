...
const BroadcastPushModal: React.FC<BroadcastPushModalProps> = ({ isOpen, onOpenChange }) => {
  const [title, setTitle] = useState("");
  const [body, setBody] = useState("");
  const [pushTemplateKey, setPushTemplateKey] = useState("");
  const [variablesString, setVariablesString] = useState(`{\n  "name": "User"\n}`);

  const [broadcastPush, { isLoading: isBroadcasting }] = useBroadcastPushMutation();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    // Check if at least one of (title and body) OR pushTemplateKey is provided
    ...
    try {
      ...
      if (pushTemplateKey.trim()) payload.pushTemplateKey = pushTemplateKey.trim();
      if (Object.keys(parsedVariables).length > 0) payload.variables = parsedVariables;

      await broadcastPush(payload).unwrap();

      toast.success("Broadcast notification sent successfully!", { id: toastId });
      onOpenChange(false);
      ...
      toast.error(err?.data?.message || err?.message || "Failed to broadcast notification", { id: toastId });
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4 flex flex-col flex-1 overflow-hidden">
          <div className="space-y-4 flex-1 overflow-y-auto pr-1">
            <div className="p-3 bg-orange-50/50 rounded-xl border border-orange-100/50 text-xs text-orange-800 leading-relaxed">
              <strong>Super Admins:</strong> Send a global push broadcast. Specify either title/body direct message OR a push template key to fetch a template with variables.
            </div>

            <div className="space-y-1">
              <Label htmlFor="broadcast-title" className="text-xs font-semibold text-gray-600 flex justify-between">
            ...
            <div className="space-y-1">
              <Label htmlFor="broadcast-body" className="text-xs font-semibold text-gray-600 flex justify-between">
            ...
            <div className="space-y-1">
              <Label htmlFor="broadcast-template-key" className="text-xs font-semibold text-gray-600 flex justify-between">
            ...
            <div className="space-y-1">
              <Label htmlFor="broadcast-variables" className="text-xs font-semibold text-gray-600 flex justify-between">
