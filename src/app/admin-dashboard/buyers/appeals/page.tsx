...
const BuyerAppealsPage = () => {
  const [selectedAppeal, setSelectedAppeal] = useState<Appeal>(mockAppealsData[0]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {mockAppealsData.map((appeal, index) => (
              <div
                key={index}
                onClick={() => handleSelectAppeal(appeal)}
                className={`p-4 rounded-lg border cursor-pointer transition-all ${
                  selectedAppeal?.id === appeal.id && selectedAppeal?.buyer === appeal.buyer
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{appeal.buyer} • {appeal.action} • {appeal.days}</p>
                  </div>
                  <button className="px-3 py-1 rounded-lg text-xs font-medium" style={{ background: "#FFF3E0", color: "#E67E22" }}>
                    {appeal.status}
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-lg font-semibold mb-4">Case Detail: {selectedAppeal.id}</h2>

          <div className="space-y-4">
            {/* Case Summary */}
            <div className="text-sm">
              <p><strong>Buyer:</strong> {selectedAppeal.buyer} | <strong>Action Appealed:</strong> {selectedAppeal.action} | <strong>Issued:</strong> {selectedAppeal.days} ago</p>
              <p className="mt-2"><strong>Reasons:</strong> {selectedAppeal.reason}</p>
              <p className="mt-2"><strong>Status:</strong> {selectedAppeal.status} | <strong>SLA:</strong> 24h to respond</p>
            </div>
            ...
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded-lg text-sm text-gray-700">
                <p>I was wrongly flagged for chargebacks. My bank confirmed the duplicate charges, reversing suspension can be lifted</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-600">Buyer email screenshot</p>
                  <button className="text-xs text-[#E67E22] mt-1">View</button>
                </div>
              </div>
            </div>
