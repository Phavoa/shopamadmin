...
export default function FinanceOverviewPage() {
  ...
  const kpiData = [
    {
      title: "GMV (Today)",
      value: isLoading ? "..." : formatNaira(stats?.gmv.today),
      subtitle: isLoading ? "" : formatPercent(stats?.gmv.percentChange ?? 0),
      subtitleColor: (stats?.gmv.percentChange ?? 0) >= 0 ? "#10B981" : "#EF4444",
    },
    {
      title: "Escrow Balance",
      value: isLoading ? "..." : formatNaira(stats?.escrowBalance),
      subtitle: "Funds held for buyers",
      subtitleColor: "#6B7280",
    },
    {
      title: "Payouts Pending",
      value: isLoading ? "..." : formatNaira(stats?.payoutsPending),
      subtitle: "Awaiting processing",
      subtitleColor: "#6B7280",
    },
    {
      title: "ShopAM Revenue",
      value: isLoading ? "..." : formatNaira(stats?.shopamRevenue.total),
      subtitle: "Commissions + fees",
      subtitleColor: "#6B7280",
    },
  ];

  // ── Bar chart — static placeholder (API has no daily history endpoint yet) ──
  // When backend adds a daily revenue endpoint, replace these heights with real data
  const dailyRevenue = [
    { day: "1", height: 130 },
    { day: "2", height: 120 },
    { day: "3", height: 90 },
  ...
  const payoutVolume = [
    { week: "GMV Today",  width: Math.round((gmvToday / maxVal) * 100),        amount: formatNaira(stats?.gmv.today) },
    { week: "GMV Yest.",  width: Math.round((gmvYesterday / maxVal) * 100),     amount: formatNaira(stats?.gmv.yesterday) },
    { week: "Pending",    width: Math.round((payoutsPending / maxVal) * 100),   amount: formatNaira(stats?.payoutsPending) },
    { week: "Livestream", width: Math.round((livestreamRevenue / maxVal) * 100),amount: formatNaira(stats?.livestreamRevenue.totalAmount) },
  ];

  // ── Finance alerts — real API data ───────────────────────────────────────────
  const financeAlerts = [
    {
      title: "Payouts Pending",
      description: `${formatNaira(stats?.payoutsPending)} awaiting processing`,
      button: "Review",
    },
    {
      title: "Escrow Balance",
      description: `Current balance: ${formatNaira(stats?.escrowBalance)}`,
      button: "View",
    },
    {
      title: "VAT Revenue",
      description: `Collected: ${formatNaira(stats?.vatRevenue)}`,
      button: "Details",
    },
    {
      title: "Active Livestreams",
      description: `${stats?.livestreams.activeCount ?? 0} live now · ${stats?.livestreams.totalViewers ?? 0} viewers`,
      button: "Monitor",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] p-">
      <div className="space-y-6">
        ...
        {/* Daily Revenue Trend — static bars, real dropdown UI */}
        {/* NOTE: replace dailyRevenue with real data once backend adds daily history endpoint */}
        <div
          style={{
            display: "flex",
            width: "1230px",
        ...
        <div className="flex gap-6">
          ...
          >
            <h2 className="text-lg font-semibold text-black">Payout Volume (Weekly)</h2>
            <div className="space-y-4 w-full">
              {payoutVolume.map((payout, index) => (
                <div key={index} className="flex items-center gap-4">
                  <p className="text-sm text-gray-700 w-20">{payout.week}</p>
                  <div className="flex-1 relative">
                    <div style={{ width: "400px", height: "14px", flexShrink: 0, borderRadius: "6px", background: "#FFF1E5" }}>
                      <div
                        style={{
                          width: isLoading ? "0%" : `${payout.width}%`,
                          height: "14px",
                          borderRadius: "6px",
                          background: "#E67E22",
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-600">
                      {isLoading ? "Loading..." : alert.description}
                    </p>
                  </div>
                  <button
                    style={{
