...
export default function FinanceOverviewPage() {
  ...
  const kpiCards = [
    {
      title: `GMV (${selectedLabel})`,
      value: kpiLoading ? "..." : formatNaira(stats?.gmv?.current),
      subtitle: kpiLoading ? "" : formatPercent(stats?.gmv?.percentChange ?? 0),
      subtitleColor:
        (stats?.gmv?.percentChange ?? 0) >= 0 ? "#10B981" : "#EF4444",
    },
    {
    ...
    },
    {
      ...
      value: kpiLoading ? "..." : formatNaira(overview?.shopamRevenue?.amount),
    ...
    : [
        {
          type: "PAYOUT",
          title: "Payouts Pending",
          subtitle: `${formatNaira(stats?.payoutsPending)} awaiting processing`,
          actionLabel: "Review",
        },
        {
          type: "ESCROW",
          title: "Escrow Balance",
          subtitle: `Current balance: ${formatNaira(stats?.escrowBalance)}`,
          actionLabel: "View",
        },
        {
          type: "VAT",
          title: "VAT Revenue",
          subtitle: `Collected: ${formatNaira(stats?.vatRevenue)}`,
          actionLabel: "Details",
        },
        {
          type: "LIVE",
          title: "Active Livestreams",
          subtitle: `${stats?.livestreams?.activeCount ?? 0} live · ${stats?.livestreams?.totalViewers ?? 0} viewers`,
          actionLabel: "Monitor",
        },
      ];
            ...
            {!isChartLoading && chartBars.length === 0 && (
              <div className="flex items-center justify-center w-full h-full">
                <p className="text-gray-400 text-sm">
                  No revenue data available for {selectedLabel}.
                </p>
              </div>
            )}
          </div>
                ...
                : payoutBars.length > 0
                  ? payoutBars.map((payout, index) => (
                      <div key={index} className="flex items-center gap-4">
                        <p className="text-sm text-gray-700 w-20 truncate">
                          {payout.label}
                        </p>
                  ...
                  : // Fallback to existing financials-API data when overview has no payoutVolume
                    (() => {
                      const gmvToday = Number(stats?.gmv?.current ?? 0);
                      const gmvYesterday = Number(stats?.gmv?.previous ?? 0);
                      const payoutsPending = Number(stats?.payoutsPending ?? 0);
                      const livestreamRevenue = Number(
                        stats?.livestreamRevenue?.totalAmount ?? 0,
                      );
                      ...
                      return [
                        {
                          label: "GMV Current",
                          width: Math.round((gmvToday / maxVal) * 100),
                          amount: formatNaira(stats?.gmv?.current),
                        },
                        {
                          label: "GMV Prev.",
                          width: Math.round((gmvYesterday / maxVal) * 100),
                          amount: formatNaira(stats?.gmv?.previous),
                        },
                        {
                          label: "Pending",
                          width: Math.round((payoutsPending / maxVal) * 100),
                          amount: formatNaira(stats?.payoutsPending),
                        },
                        {
                          label: "Livestream",
                          width: Math.round((livestreamRevenue / maxVal) * 100),
                          amount: formatNaira(
                            stats?.livestreamRevenue?.totalAmount,
                          ),
                        },
                      ].map((row, i) => (
                        <div key={i} className="flex items-center gap-4">
                          <p className="text-sm text-gray-700 w-20">
                            {row.label}
                          </p>
                          <div className="flex-1 relative">
                            <div
